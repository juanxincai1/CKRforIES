...
with tf.Session() as sess:
    ...
    days_rewards_best=-10000000
    days_rewards_last=[]
    days_rewards_best_list=[]
    EP_reward_train_list = []
    EP_reward_test_list = []
    for loop in range(episode1):
        for qwe in range(1):
            ...
            print('EP_reward=', EP_reward, 'days_rewards_best=', days_rewards_best, 'episodes=', i,'contrast_episode=', qwe,'contrast_round=',loop)
        days_rewards_last.append(np.mean(EP_reward_list))
        days_rewards_best_list.append(days_rewards_best)
    EP_reward_test=0
    EP_reward_test_list=[]
    ...
    a=max(EP_reward_test_list)
    b=min(EP_reward_test_list)
    c=np.std(EP_reward_test_list)
    d=np.mean(EP_reward_test_list)
    print("max:%s,min:%s,std:%s,average:%s"%(a,b,c,d))
    aa=max(EP_current_cost_test_list)
    bb=min(EP_current_cost_test_list)
    cc=np.std(EP_current_cost_test_list)
    dd=np.mean(EP_current_cost_test_list)
    print("max:%s,min:%s,std:%s,average:%s"%(aa,bb,cc,dd))
